...
export default function Index() {
  ...
  const [activeTab, setActiveTab] = useState<'editor' | 'cloud'>('editor');
  ...
  useEffect(() => {
    if (activeTab === 'cloud') loadCloudPpts();
  }, [activeTab, loadCloudPpts]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 border-l border-border pl-4">
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-md font-medium transition-colors ${activeTab === 'editor' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                编辑器
              </button>
              <button
                onClick={() => setActiveTab('cloud')}
                className={`text-xs px-3 py-1.5 rounded-md font-medium transition-colors ${activeTab === 'cloud' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                云端PPT
              </button>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {activeTab === 'editor' && (
              <>
                ...
                <Button onClick={handleExportCloud} disabled={!hasContent || exporting} variant="outline" size="sm">
      ...
      {activeTab === 'editor' ? (
        ...
        <div className="max-w-3xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-lg font-bold">云端PPT</h2>
            <Button variant="ghost" size="sm" onClick={loadCloudPpts} disabled={loadingCloud}>
              <RefreshCw className={`w-4 h-4 mr-1 ${loadingCloud ? 'animate-spin' : ''}`} />
              刷新
            </Button>
          </div>
          <p className="text-xs text-muted-foreground mb-4">云端PPT保存10天后自动清理</p>
          {loadingCloud ? (
            ...
            <div className="space-y-2">
              {cloudPpts.map(pptx => (
                <div key={pptx.id} className="flex items-center justify-between p-4 rounded-lg border border-border bg-card/50">
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-2 py-2">
            ...
            <Input
              ...
              placeholder='例如：6.7上午'
            />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowSaveDialog(false)}>取消</Button>
            <Button onClick={confirmCloudSave} disabled={exporting}>
              {exporting ? '保存中...' : '确认保存'}
            </Button>
          </DialogFooter>
        </DialogContent>
